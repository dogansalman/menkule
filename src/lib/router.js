...
function Router(routerConfig) {
  ...
  this.routerConfig.forEach(c => {
      ...
      const reqValidateResults =  typeof c.slice(1, 2)[0] == "function" && c.length === 4 ? c.slice(1, 2)[0]() : App.promise(() => false);
      reqValidateResults.then((r) => {
          window.deneme = r;
      });

      const reqValidateResult = window.deneme;

      console.log(reqValidateResult);
